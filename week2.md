...
<details>
<summary> 진심 어린 감정을 유머 있게 전달하기 </summary>

![](https://velog.velcdn.com/images/flowersayo/post/75fca93c-1360-4227-8c2f-8c54aebbe004/image.png)
![](https://velog.velcdn.com/images/flowersayo/post/1c55b4a2-1a06-484b-a544-7e035a1c15ce/image.png)

이번주 스터디를 마무리 하고 각자 팀원들에게 전하고 싶은 진심을 정리했씁니다.
그리고 GPT 에게 '팀원들에게 전달하고 싶은 진정성 있는 마음을 AI를 통해 재치있게 공유하는식으로 바꿔주라.' 하고 프롬프트를 보냈습니다.
그런데 그렇게 보냈더니 아직 진지 60 + 재치 40 느낌이라 기가채드 라는 인물형 AI 에게 본인 스타일대로 바꿔달라고해서 조금 더 재치있고 자극적인 메시지가 완성되었던 것 같습니다.
...
평소에 클라우드에 관해 관심이 있었는데, 아무래도 각 서비스사 별로 나뉘어져 있다 보니 정보를 취합하기가 쉽지 않았습니다. 

```
클라우드 - AWS, Azure, Naver cloud, 기타 - 상위 10개의 의 2024~2025년 기준 주요 이슈 및 트렌드를 아래 항목에 따라 체계적으로 분석해줘.

1. 클라우드 별 주요 특징 및 대표 장단점
2. 클라우드 별 최근 토픽
3. 최근 논란 및 문제점
4. 향후 발전 전망 및 주목해야 할 기능

각 문항마다 관련 예시나 수치를 2~3줄 이내로 요약해서, 마크다운 표와 목록을 섞어 정리해줘.
추가적으로 클라우드의 점유율 전망 통계를 더 분석해줘.
```
와 같이 프롬프트를 AI에게 질문하는것 보다 원하는 것을 찾는다는 느낌으로 구체적인 예시를 들어 전달하니 표와 함께 다음과 같이 잘 정리된 정보를 얻을 수 있었습니다.

<img width="1006" height="1755" alt="FireShot Capture 005 - Google Gemini -  gemini google com" src="https://github.com/user-attachments/assets/499be609-51d2-4d32-8375-30280045d12a" />


AI를 활용해보니 제가 직접 수집하는것 보다 한눈에 보기 쉽게 요약해주어서 유용했던 것 같습니다.

- 4번 퀘스트 수행 기록

<img width="774" height="727" alt="image" src="https://github.com/user-attachments/assets/37e47ed8-b5c1-49d6-83cf-84f601399c28" />


예시와 비슷하게, 전달하고 싶은 생각을 AI를 통해 재치있게 전달하려고 만들어봤습니다. 

처음 제 생각을 입력하고 `재치있는 말투로 만들어줘!`라고 하니 그렇게까지 말투가 바뀌지는 않았는데, AI에게 확실한 역할을 부여해서 `너는 이러한 인물인데 어떠한 입장에서 재미있게 말해줘!`라고 입력하니 위와 같이 재미있으면서도 도움이 되는 답변을 받을 수 있었습니다.

**📌 느낀점**

너무 진지하게 마음을 전달하게 되면 쉽게 루즈해질 수 있는데, AI 를 활용해서 '재치' 있게 표현을 바꿀 수 있다는 점이 신기했고
도움이 많이 되었습니다.
</details>
...
#### S024_이상유
**📌 실행한 퀘스트: 팀원들에게 전달하고 싶은 진정성 있는 마음을 AI를 통해 재치있게 공유하기**

**📌 사용한 AI**: Chat GPT

**📌 실행 과정**

이 퀘스트 특성에도 그렇지만 AI에게 역할을 부여하는 게 답변의 수준을 올리거나 원하는 방향으로 더 정확한 답변을 얻을 수 있게 도와준다는 걸 AI 피어 세션부터 느껴왔습니다.

재치있게 전달해보고 싶었는데, 최신 밈에 조금 약해서 AI에게 어떤 역할을 부여할지 고민했습니다.

긍정적인 마인드를 공유하고 싶다는 생각에 **‘럭키비키하게’** 제 말을 전달하는 걸 도와달라고 했습니다.

**프롬프트 내용을 설명해보자면,** 우선 현재 팀원들이 상황이 어떤지를 설명했습니다. 

뭐 어떤 미션을 하고 있는지, 지금 어떤 힘듦을 겪고 있을지도 말했습니다.
...
답변을 받고 추가로 고쳐달라는 요청을 여러 번 했습니다.

```
(첫 프롬프트) -> 좀 더 럭키비키 해줘 -> 좀 짧게 해줘 -> 이것보다는 길게 해줘
```

이 퀘스트에서 제가 얻을 답변은 제가 전달하려는 마음이기 때문에 **내 생각이 정답이다라는** 마음으로 AI의 답변을 수정하려는 시도를 했습니다.

학습을 위해 AI를 사용할 때는 보통 수동적으로 전달을 받는데, 계속 이게 맞는지 아닌지 확인하고 물어보는 과정이 필요하다는 걸 이어서 생각하게 되었습니다.

그래도 AI를 사용해서 제가 생각하지 못했던 문장이나 표현을 찾을 수 있다는 점이 좋기도 했습니다.

## 추가 토의 내용✍️
-🤖AI의 역할 부여!
  - 위의 미션들을 수행한 결과, 공통적으로 모인 의견으로는, 구체적인 AI의 역할 설정 이었습니다
  - 역할을 부여하면 방향성이 더 구체적이고 사용자의 의도에 맞게 동작할 수 있다는 결론을 얻었습니다.
  - 또 반대로 색다른 역할을 부여하면 예측하기 힘든 뛰어난 결과물을 도출할 수 있다는 결론도 얻었습니다
